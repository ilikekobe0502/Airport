...
public final class MyApplication extends android.support.multidex.MultiDexApplication implements GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks, LocationListener
{
    ...
    {
        if (BuildConfig.isOverlay && m_needOutdoorSearch && m_googleApiClient != null)
        {
    ...
    public Manager.Location getUserLocation(float[] retBearing)
    {
        ...
        final int ordinal = 0;
        return new Manager.Location(null, -1, new Coordinate3D(latitude, longitude, ordinal), null, getString(R.string.current_location));
    }
}
